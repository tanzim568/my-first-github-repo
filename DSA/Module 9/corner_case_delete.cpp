...
void delete_head(Node* &head,Node* &tail)
{
    Node* delNode=head;
    head=head->next;
    // head er next null so head ekhon NULL e 
    delete delNode;
    if(head==NULL)
    {
        tail = NULL;
    ...
    head->prev=NULL;
    // jehetu head e NULL e ache tai head er prev korte null er prev e korle segmentation fault asbe
}
int size(Node* head)
{
    Node *tmp=head;
...
/* jodi emon hoi je head delete korte korte 1 node ache tail o 1 node means head ei ache  ba first thekei  1 node ache tokhon error asto ekhne amra dleete kori je tmp*/
